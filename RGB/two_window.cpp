...
// d.mocap_pos[m.body_mocapid[mujoco.mj_name2id(m, mujoco.mjtObj.mjOBJ_BODY, 'target')], :][1] = next_target_depth
  ...
  while (!glfwWindowShouldClose(window))
  {
    ...
    camera_pos[camera_body_id*3 + 1] = sin(i/60.0);
...
void RGBD_sensor(mjModel* model, mjData* data)
{
  ...
  // rgbd_camera.fixedcamid = mj_name2id(model, mjOBJ_CAMERA, "egocentric");
  ...
  while (!glfwWindowShouldClose(window))
  {
    ...
    /*if (mj_RGBD.color_buffer && mj_RGBD.depth_buffer) {
      std::cout << "Color and depth buffers are filled." << std::endl;
    } else {
      std::cout << "Buffers are empty or uninitialized." << std::endl;
    }
    mjr_drawPixels(mj_RGBD.color_buffer, mj_RGBD.depth_buffer,viewport, &sensor_context);*/
...
int main(int argc, char** argv)
{
  ...
  const char* file = R"(C:\Project\mujoco\celeste\RGB\data\camera_test.xml)";
  model = mj_loadXML(file, nullptr, error, 1000);
