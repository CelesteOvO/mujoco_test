...
inline uchar* color_buffer{};    // color buffer
inline float* depth_buffer{};  // depth buffer

inline cv::Mat color_image;
inline cv::Mat depth_image;

// OpenGL render range
inline double extent;  // depth scale (m)
inline double z_near;  // near clipping plane depth
inline double z_far;   // far clipping plane depth
// camera intrinsics
inline double f;   // focal length
inline int cx, cy; // principal points
inline std::mutex myMtx;
inline pcl::PointCloud<pcl::PointXYZRGB>::Ptr color_cloud{new pcl::PointCloud<pcl::PointXYZRGB>};
...
class Simulate {
 public:
  ...
  static pcl::PointCloud<pcl::PointXYZRGB> generate_color_pointcloud();
