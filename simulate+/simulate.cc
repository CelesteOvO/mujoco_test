...
void Simulate::set_camera_intrinsics(const mjModel* model, const mjvCamera camera, const mjrRect viewport)
{
  // vertical FOV
...
cv::Mat Simulate::linearize_depth(const cv::Mat& depth)
{
  cv::Mat depth_img(depth.size(), CV_32F, cv::Scalar(0));

  for (unsigned int i = 0; i < depth_img.rows; i++)
  {
    auto* raw_depth_ptr = depth.ptr<float>(i);
    auto* m_depth_ptr = depth_img.ptr<float>(i);

    for (unsigned int j = 0; j < depth_img.cols; j++)
    {
...
void Simulate::get_RGBD_buffer(const mjModel* model, const mjrRect viewport, const mjrContext* context)
{
  ...
  // mjr_drawPixels(color_buffer, depth_buffer,viewport, context);
  ...
  cv::normalize(depth_img_m, depth_normalized, 0, 255, cv::NORM_MINMAX);
  depth_normalized.convertTo(depth_normalized, CV_8U);
  cv::imshow("depth_normalized", depth_normalized);
  depth_img_m.copyTo(depth_image);
}

pcl::PointCloud<pcl::PointXYZRGB> Simulate::generate_color_pointcloud()
{
  using namespace pcl;
...
void Simulate::InitImGui() const {

  // if (!gladLoadGLLoader(reinterpret_cast<GLADloadproc>(glfwGetProcAddress))) {
  //   throw std::runtime_error("glad initialization OpenGL failure");
  // }
  auto* window = static_cast<GLFWwindow*>(this->platform_ui->GetUserWindowPointer());
  float scaleY{};
  glfwGetWindowContentScale(window, nullptr, &scaleY);
  IMGUI_CHECKVERSION();
  ImGui::CreateContext();
  ...
  io.Fonts->AddFontFromFileTTF(R"(C:\Windows\Fonts\consola.ttf)", 16 * scaleY, nullptr,
...
void Simulate::DrawImGui() {
  ...
  // // 2. 从当前帧缓冲区直接复制数据到 GPU 纹理
  // GLint currentFBO{};
  // glGetIntegerv(GL_FRAMEBUFFER_BINDING, &currentFBO);
  // glBindFramebuffer(GL_FRAMEBUFFER, 0); // 确保绑定到默认帧缓冲区
  // glCopyTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, 0, 0, 100, 100, 0);
  // glBindFramebuffer(GL_FRAMEBUFFER, currentFBO);

  mjrRect viewport = this->uistate.rect[3];
  float small_windpw_width = 0.4;
  // mjrRect viewport2 = {
  //   static_cast<int>(viewport.left + viewport.width * (1 - small_windpw_width)),
  //   static_cast<int>(viewport.bottom),
  //   static_cast<int>(viewport.width * small_windpw_width),
  //   static_cast<int>(viewport.height * small_windpw_width)
  // };
  mjrRect viewport2 = {
    0,
    0,
    this->platform_ui->mjr_context().offWidth,
    this->platform_ui->mjr_context().offHeight
  };
  mjvCamera rgbd_camera = [&]()
  {
    mjvCamera cam{};
    cam.type = mjCAMERA_FIXED;
    cam.fixedcamid = mj_name2id(this->m_, mjOBJ_CAMERA, "camera");
    return cam;
  }();

  if(rgbd_camera.fixedcamid == -1) {
    return;
  }

  static std::vector<unsigned char> rgbData(viewport2.width * viewport2.height * 3);
  static std::vector<float> depthData(viewport2.width * viewport2.height);
  static std::vector<unsigned char> depthColorData(viewport2.width * viewport2.height * 3);

  if (rgbData.size() != viewport2.width * viewport2.height * 3) {
    rgbData.resize(viewport2.width * viewport2.height * 3);
    depthData.resize(viewport2.width * viewport2.height);
    depthColorData.resize(viewport2.width * viewport2.height * 3);
  }


  mjv_updateScene(this->m_, this->d_, &this->opt, nullptr, &rgbd_camera, mjCAT_ALL, &this->scn);
  mjr_setBuffer(mjFB_OFFSCREEN, &this->platform_ui->mjr_context());
  mjr_render(viewport2, &this->scn, &this->platform_ui->mjr_context());

  set_camera_intrinsics(this->m_, rgbd_camera, viewport2);
  // std::cout << "f: " << f << ", cx: " << cx << ", cy: " << cy << std::endl;
  get_RGBD_buffer(this->m_, viewport2, &this->platform_ui->mjr_context());
  myMtx.lock();
  *color_cloud = generate_color_pointcloud();
  myMtx.unlock();

  // GLuint fbo;
  // glGetIntegerv(GL_FRAMEBUFFER_BINDING, (GLint*)&fbo);
  // GLint textureID_bak;
  // glBindFramebuffer(GL_FRAMEBUFFER, fbo);  // 绑定当前FBO
  // glGetFramebufferAttachmentParameteriv(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_FRAMEBUFFER_ATTACHMENT_OBJECT_NAME, &textureID_bak);
  //
  // glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, textureID, 0);
  //
  // mjr_render(viewport2, &this->scn, &this->platform_ui->mjr_context());
  //
  // glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, textureID_bak, 0);
  ...
  GLuint tex_bak2{};
  glGetIntegerv(GL_TEXTURE_BINDING_2D, (GLint*)&tex_bak2);
  glBindTexture(GL_TEXTURE_2D, textureID);
  // glViewport(0, 0, viewport2.width, viewport2.height);

  glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, viewport2.width, viewport2.height, 0, GL_RGB, GL_UNSIGNED_BYTE, nullptr); // 绑定纹理

  {
    GLbitfield mask = GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT;
    auto* con = &this->platform_ui->mjr_context();
    if (con->offSamples)
    {
      ...
      // resolve-blit
      glBlitFramebuffer(viewport2.left, viewport2.bottom,
                        viewport2.left+viewport2.width, viewport2.bottom+viewport2.height,
                        viewport2.left, viewport2.bottom,
                        viewport2.left+viewport2.width, viewport2.bottom+viewport2.height,
                        mask, GL_NEAREST);
      err = glGetError();
      if (err != GL_NO_ERROR)
      {
        ...
        case GL_INVALID_ENUM:
          printf("GL_INVALID_ENUM error\n");
          break;
        case GL_INVALID_VALUE:
          printf("GL_INVALID_VALUE error\n");
          break;
        case GL_INVALID_OPERATION:
          printf("GL_INVALID_OPERATION error\n");
          break;
        case GL_INVALID_FRAMEBUFFER_OPERATION:
          printf("GL_INVALID_FRAMEBUFFER_OPERATION error\n");
          break;
        case GL_OUT_OF_MEMORY:
          printf("GL_OUT_OF_MEMORY error\n");
          break;
        default:
  ...
  glCopyTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, 0, 0, viewport2.width, viewport2.height);
  // TODO: 把深度缓冲区也读到纹理；写一个shader转换深度缓冲区到rbg；写一个类，一个相机分配一个fbo；
  // glCopyTexImage2D(GL_TEXTURE_2D, 0,GL_RGB, 0, 0, viewport2.width, viewport2.height, 0);
  mjr_restoreBuffer(&this->platform_ui->mjr_context());
  ...
  glBindTexture(GL_TEXTURE_2D, tex_bak2);


  mjr_readPixels(rgbData.data(), depthData.data(), viewport2, &this->platform_ui->mjr_context());
  mjr_setBuffer(mjFB_WINDOW, &this->platform_ui->mjr_context());


  float znear = this->m_->vis.map.znear;
  float zfar = this->m_->vis.map.zfar / 10;
  // printf("znear: %f, zfar: %f\n", znear, zfar);
  // auto xxx = depthData[0];
  for (size_t i = 0; i < depthData.size(); ++i) {
    // if(std::abs(depthData[i] - xxx) > 0.4) {
    //   printf("depthData[%d] = %f\n", i, depthData[i]);
    // }
    float z = depthData[i] * 2.0 - 1.0;
    float linearDepth = (2.0 * znear * zfar) / (zfar + znear - z * (zfar - znear));
    // float linearDepth = depthData[i];
    linearDepth = std::clamp(linearDepth, 0.0f, 1.0f);
    unsigned char colorValue = static_cast<unsigned char>(linearDepth / zfar * 255);

    // auto z = znear / (1.0f - depthData[i] * (1.0f - znear / zfar));
    // unsigned char colorValue = z * 255;

    depthColorData[i * 3] = colorValue;        // R
    depthColorData[i * 3 + 1] = colorValue;    // G
    depthColorData[i * 3 + 2] = colorValue;    // B
  }

  // GLint currentFBO{};
  // glGetIntegerv(GL_FRAMEBUFFER_BINDING, &currentFBO);
  // glBindFramebuffer(GL_READ_FRAMEBUFFER, this->platform_ui->mjr_context().offFBO);
  // glReadBuffer(GL_COLOR_ATTACHMENT0);

  // printf("%d %d\n", this->platform_ui->mjr_context().offWidth, this->platform_ui->mjr_context().offHeight);
  //
  // GLint width;
  ...
  GLuint tex_bak{};
  glGetIntegerv(GL_TEXTURE_BINDING_2D, (GLint*)&tex_bak);
  glBindTexture(GL_TEXTURE_2D, textureID);
  // glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, viewport2.width, viewport2.height, 0, GL_RGB, GL_UNSIGNED_BYTE, rgbData.data());
  // // glCopyTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, viewport.left, viewport.bottom, viewport.width, viewport.height, 0);
  // glCopyTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, viewport2.left, viewport2.bottom, viewport2.width, viewport2.height, 0);
  glBindTexture(GL_TEXTURE_2D, textureID2);
  glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, viewport2.width, viewport2.height, 0, GL_RGB, GL_UNSIGNED_BYTE, depthColorData.data());
  ...
  static ImVec2 imgSize = [&]()
  {
    ...
    float scaleY{};
    glfwGetWindowContentScale(window, nullptr, &scaleY);
    const ImVec2 imgSize_(500 * scaleY, viewport2.height * 500 * scaleY / viewport2.width);
    ImGui::SetNextWindowSize(ImVec2(imgSize_.x + 20, imgSize_.y * 2 + 150));
    return imgSize_;
  }();

  ImGui::Begin("Img");

  auto imWindowSize = ImGui::GetWindowSize();
  imgSize = {imWindowSize.x - 20, viewport2.height * (imWindowSize.x - 20) / viewport2.width};

  ImGui::Text("RGB");
  ImGui::Image(static_cast<ImTextureID>(
                 static_cast<intptr_t>(textureID)),
               imgSize,
               ImVec2(0.0f, 1.0f),
               ImVec2(1.0f, 0.0f));
  ImGui::End();
  ...
  ImGui::Text("Depth");
  ImGui::Image(static_cast<ImTextureID>(
                 static_cast<intptr_t>(textureID2)),
               imgSize,
               ImVec2(0.0f, 1.0f),
               ImVec2(1.0f, 0.0f));
  ImGui::End();
